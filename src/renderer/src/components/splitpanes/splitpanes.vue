...
export default {
  ...
  props: {
    ...
    mode: { type: String, default: 'normal' } // drawer
  },
  ...
  methods: {
    ...
    onMouseMove (event) {
      if (this.touch.mouseDown) {
        ...
        if ((!this.horizontal && movementX === 0) || (this.horizontal && movementY === 0)) return
        this.touch.dragging = true
        this.calculatePanesSize(this.getCurrentMouseDrag(event))

        this.updateDrawerSplitterArrow()
    ...
    onDrawerArrowClick (event, splitterIndex) {
      ...
      const siblings = {
        paneBefore: this.panes[splitterIndex - 1],
        paneAfter: this.panes[splitterIndex],
      ...
      let prevAllowSize = this.pushOtherPanes ? 100 : 100 - siblings.prevPanesSize

      // 判断后一个pane的宽度或者高度是否是0或者小于其默认宽度
      const { size, expandSize } = siblings.paneAfter

      const _expandSize = expandSize / containerSize * 100

      if (size <= 0) {
        prevAllowSize = Math.min(_expandSize, prevAllowSize)
      }
      else {
        if (size < _expandSize) {
          prevAllowSize = Math.min(_expandSize, prevAllowSize) - size
        }
        else if (size >= _expandSize) {
          // 收起来
          // prevAllowSize = -size
          siblings.paneBefore.size += siblings.paneAfter.size
          siblings.paneAfter.size = 0
      ...
      const prevAllowPixel = prevAllowSize / 100 * containerSize

      const rect = this.container.getBoundingClientRect()
      const { clientX, clientY } = ('ontouchstart' in window && event.touches) ? event.touches[0] : event

      const drag = {
        x: clientX - rect.left - prevAllowPixel,
        y: clientY - rect.top - prevAllowPixel
      }

      // console.log(drag)

      this.calculatePanesSize(drag)

      this.updateDrawerSplitterArrow()
      this.$emit('resized', this.panes.map(pane => ({ min: pane.min, max: pane.max, size: pane.size })))
    },

    // calcOffset (event, splitterIndex) {
    //   const targetElement = this.container.querySelectorAll('.splitpanes__splitter')[splitterIndex - 1] // 替换为你的目标元素的 ID
    //   const targetRect = targetElement.getBoundingClientRect() // 获取目标元素的边界框位置

    //   const mouseX = event.clientX // 鼠标点击的 X 坐标
    //   const mouseY = event.clientY // 鼠标点击的 Y 坐标

    //   // 计算鼠标点击位置与目标元素的水平和垂直距离
    //   const distanceX = Math.abs(mouseX - targetRect.left)
    //   const distanceY = Math.abs(mouseY - targetRect.top)

    //   return {
    //     calcX: event.clientX - distanceX,
    //     calcY: event.clientY - distanceY
    //   }
    // },

    updateDrawerSplitterArrow () {
      if (this.mode !== 'drawer') return

      const splitters = this.container.querySelectorAll(':scope > .splitpanes__splitter .splitpanes__splitter__arrow')
      const containerSize = this.container[this.horizontal ? 'clientHeight' : 'clientWidth']

      this.panes.forEach((pane, paneIndex) => {
        if (paneIndex === 0) return

        const { size, expandSize } = pane

        const _expandSize = expandSize / containerSize * 100

        if (size >= _expandSize) {
          splitters[paneIndex - 1].classList.add('arrow__collapse')
        }
        else {
          if (splitters[paneIndex - 1].classList.contains('arrow__collapse')) splitters[paneIndex - 1].classList.remove('arrow__collapse')
        }
      })
    },
    ...
    addSplitter (paneIndex, nextPaneNode, isVeryFirst = false) {
      ...
      splitterElem.classList.add('splitpanes__splitter')

      // 绑定事件
      if (!isVeryFirst) {
        splitterElem.onmousedown = event => this.onMouseDown(event, splitterIndex)

        if (typeof window !== 'undefined' && 'ontouchstart' in window) {
          splitterElem.ontouchstart = event => this.onMouseDown(event, splitterIndex)
        }
        splitterElem.onclick = event => this.onSplitterClick(event, splitterIndex + 1)
      }

      if (this.dblClickSplitter) {
        splitterElem.ondblclick = event => this.onSplitterDblClick(event, splitterIndex + 1)
      }

      if (this.mode === 'drawer') {
        const drawerArrow = document.createElement('div')
        drawerArrow.classList.add('splitpanes__splitter__arrow')
        drawerArrow.onclick = event => this.onDrawerArrowClick(event, splitterIndex + 1)
        // drawerArrow.onmousedown = drawerArrow.onmousemove = (event) => {
        //   event.stopPropagation()
        //   event.preventDefault()
        // }
        splitterElem.appendChild(drawerArrow)
      }

      nextPaneNode.parentNode.insertBefore(splitterElem, nextPaneNode)
    },
    ...
    onPaneAdd (pane) {
      ...
      const expandSize = parseFloat(pane.expandSize)
      // const _expandSize =
      this.panes.splice(index, 0, {
        ...
        givenSize: pane.size,
        // expandSize: parseFloat(expandSize) || max || pane.size || 100,
        expandSize: parseFloat(expandSize) || max || pane.size || 100,
        update: pane.update
      })
    ...
    return h(
      'div',
      {
        ...
        class: [
          'splitpanes',
          {
            'splitpanes--drawer': this.mode === 'drawer'
          },
...
<style lang="scss">
@use './css/splitpanes.scss';
// .splitpanes {
//   --pane-background-color: #f2f2f2;
// }
...
//       background-color: #ccc;
//       cursor: pointer;
//       box-shadow: 2px 6px 10px 0px #5e606129;
//       &:before, &:after {
//         content: '';
//         position: absolute;
//         right: 6px;
